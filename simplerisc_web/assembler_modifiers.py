...
MODIFIERS = {"h": "10", "u": "01", "default": "00"}  # Modifier bits for immediate values
...
def assemble_instruction(instruction, labels, current_address):
    ...
    base_opcode = tokens[0].rstrip("hu")  # Remove h or u suffix
    opcode = OPCODES.get(base_opcode)
    if opcode is None:
        raise ValueError(f"Invalid opcode: {tokens[0]}")
    
    modifier_bits = MODIFIERS.get(tokens[0][-1], "00")  # Determine modifier from opcode suffix
    is_immediate = "0"
    rd_bin = "0000"
    r1_bin = "0000"
    r2_bin = "0000"
    ...
    if base_opcode in ["add", "sub", "mul", "div", "mod", "and", "or", "lsl", "lsr", "asr"]:
        ...
        if r2_or_imm in REGISTERS:
            ...
            return (opcode + "0" + rd_bin + r1_bin + r2_bin + modifier_bits + "000000000000").zfill(32)
        else:
            is_immediate = "1"
            imm_bin = modifier_bits + format(int(r2_or_imm), '016b')
            return (opcode + is_immediate + rd_bin + r1_bin + imm_bin).zfill(32)

    elif base_opcode == "cmp":
        r1, r2_or_imm = tokens[1], tokens[2]
        r1_bin = REGISTERS[r1]
        if r2_or_imm in REGISTERS:
            r2_bin = REGISTERS[r2_or_imm]
            return (opcode + "0" + "0000" + r1_bin + r2_bin + modifier_bits + "000000000000").zfill(32)
        else:
            imm_bin = modifier_bits + format(int(r2_or_imm), '016b')
            return (opcode + "1" + "0000" + r1_bin + imm_bin).zfill(32)

    elif base_opcode in ["mov","not"]:
        rd, r2_or_imm = tokens[1], tokens[2]
        rd_bin = REGISTERS[rd]
        if r2_or_imm in REGISTERS:
            r2_bin = REGISTERS[r2_or_imm]
            return (opcode + "0" + rd_bin + "0000" + r2_bin + modifier_bits + "000000000000").zfill(32)
        else:
            imm_bin = modifier_bits + format(int(r2_or_imm), '016b')
            return (opcode + "1" + rd_bin + "0000" + imm_bin).zfill(32)


    elif base_opcode in ["ld", "st"]:
        rd, addr = tokens[1], tokens[2]
        rd_bin = REGISTERS[rd]
        imm_bin = format(int(addr), '018b')
        is_immediate = "1"
        return (opcode + is_immediate + rd_bin + addr + modifier_bits + "000000000000")

    elif base_opcode in ["bgt", "beq"]:
        label = tokens[1]
        if label in labels:
    ...
    elif base_opcode in ["hlt", "end", "nop", "ret"]:
